...
# Número de movimientos de la cuenta 1 (Sale 0 ya que no tiene).
SELECT COUNT(*)
FROM movimiento
WHERE cueNumero = 1;

# Saldo real o suma de importes de movimientos de la cuenta 1001 (Saldrá NULL si no tubuera).
SELECT SUM(importe)
FROM movimiento
WHERE cueNumero=1001;

# Ver saldo y suma de importes (movimiento). Solo de las cuentas que tienen movimiento
SELECT cuenta.numero, cuenta.saldo, SUM(movimiento.importe)
FROM cuenta INNER JOIN movimiento ON cuenta.numero=movimiento.cueNumero 
GROUP BY cuenta.numero, cuenta.saldo;

# Ver saldo y suma de importes (movimiento).
SELECT cuenta.numero, cuenta.saldo, SUM(movimiento.importe)
FROM cuenta LEFT JOIN movimiento ON cuenta.numero=movimiento.cueNumero 
GROUP BY cuenta.numero, cuenta.saldo;

# Ver saldo y suma de importes (movimiento) con 0 por NULL (CASE WHEN ...)
SELECT cuenta.numero, cuenta.saldo, CASE WHEN SUM(movimiento.importe) IS NULL THEN 0 ELSE SUM(movimiento.importe) END
FROM cuenta LEFT JOIN movimiento ON cuenta.numero=movimiento.cueNumero 
GROUP BY cuenta.numero, cuenta.saldo; 

# Ver saldo y suma de importes (movimiento) con 0 por NULL (COALESCE).
SELECT cuenta.numero, cuenta.saldo, COALESCE(SUM(movimiento.importe),0)
FROM cuenta LEFT JOIN movimiento ON cuenta.numero=movimiento.cueNumero 
GROUP BY cuenta.numero, cuenta.saldo; 

# Igual con el saldoMinimo
SELECT cuenta.numero, cuenta.saldo, cuenta.saldominimo, COALESCE(SUM(movimiento.importe),0)
FROM cuenta LEFT JOIN movimiento ON cuenta.numero=movimiento.cueNumero 
GROUP BY cuenta.numero, cuenta.saldo, cuenta.saldominimo;

# Ver saldo y CONTAR los movimientos.
SELECT cuenta.numero, cuenta.saldo, COALESCE(COUNT(movimiento.cueNumero),0)
FROM cuenta LEFT JOIN movimiento ON cuenta.numero=movimiento.cueNumero 
GROUP BY cuenta.numero, cuenta.saldo;
